...
public class AppsIndexView extends LinearLayout {
    ...
    public void setSwipeEditLayout(RgkSateLiteFavoriteDialog swipeEditFavoriteDialog) {
        mSwipeEditFavoriteDialog = swipeEditFavoriteDialog;
    }

    public void setContent(ArrayList<RgkItemAppsInfo> infos, ArrayList<RgkItemAppsInfo> headerlist) {
        mAppsGridLayout.removeAllViews();
        GridLayoutItemView itemview;
        for (int i = 0; i < infos.size(); i++) {
            //加载子布局
            itemview = (GridLayoutItemView) LayoutInflater.from(getContext()).inflate(R.layout.rgk_grid_item_layout, null);
            ...
            mAppsGridLayout.addView(itemview, mAppsGridLayout.getChildCount(), new
                    LayoutParams(mSize, mSize));
        }
    }

    public boolean containApp(ArrayList<RgkItemAppsInfo> applist, RgkItemAppsInfo app) {
